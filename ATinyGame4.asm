...
memoryInit:
	...
	mov r27, r30 ;store the first number in the sequence so it can be recreated
	
	ldi r17, 1 ;the score (length of the memory sequence)
	;it is 1 higher than the actual number of presses
	ldi r29, 1 ;the number of items left in the current sequence
...
memoryShowBetween:
	...
	dec r29 ;decrement sequence index
	...
	mov r29, r17 ;reset the sequence index to the sequence length (score)
...
stackerMove:
	...
	;if the timer is 
	mov r30, r25
	sub r30, r29 ;check if the delay has elapsed yet
	brne statesEnd2
	clr r25
...
statesEnd:
	...
	;push r30 ;r30 and r31 are always tmp and don't need to be saved
	;push r31
	push r18 ;r18 is the current state and needs to be saved

	;push r26 ;r26-r17 are not touched by the code after this so they will 
